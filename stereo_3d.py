...
def calculate_point_operation(xy, frame1, frame2):
    ...
    for xy_temp in xy:
        if len(xy_temp) == 2:
            try:
                # 坐标取整和边界检查
                x = int(round(xy_temp[0]))
                y = int(round(xy_temp[1]))

                if y < 0 or y >= threeD.shape[0]:
                    continue
                if x < 0 or x >= threeD.shape[1]:
                    continue
                ...
                # 有效性检查（排除无效点）
                if np.any(np.abs(point_cam) > 1e6):
                    point_world.append([-1, -1, -1])
                    continue

                point_world.append((point_cam / 1000).tolist())
            except Exception as e:
...
if __name__ == '__main__':
    while True:
        ...
        # 设置鼠标回调
        # cv2.setMouseCallback("right", onmouse_pick_points, (imgL_rectified, imgR_rectified))
        # imgL_display = visualize_ground_plane(
        #     imgL_display,
        #     transformer,
        #     left_camera_matrix,
        #     left_distortion
