...
# filepath: [xt6.py](http://_vscodecontentref_/1)
def calculate_point_operation(xy, frame1, frame2):
    """
    三维坐标处理函数 - 兼容stereo_3d.py接口
    ...
    """
    global transformer  # 使用全局transformer变量
    point_world = []
    ...
    try:
        ...
        print(f"生成3D点云时发生错误: {e}")
        return [[-3, -3, -3]] * len(xy)
    
    # 确保threeD和transformer有效
        ...
        if isinstance(xy_temp, (list, tuple)) and len(xy_temp) == 2:
            ...
            if 0 <= y < height and 0 <= x < width:
                ...
                if np.all(np.isfinite(point_cam)) and not np.any(np.abs(point_cam) > 1e6):
                    try:
                        # 坐标转换
                        transformed_point = transformer.transform(point_cam) / 1000.0  # 转换为米
                        point_world.append(transformed_point.tolist())
                    except ValueError as e:
                        # 坐标转换错误
                        point_world.append([-2, -2, -2])
                    except Exception as e:
                        # 其他错误
                        point_world.append([-3, -3, -3])
                else:
                    # 无效深度点
