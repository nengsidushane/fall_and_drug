...
# --- Even More Aggressive SGBM Parameters ---
stereo = cv2.StereoSGBM_create(
    minDisparity=0,
    numDisparities=32,  # Further reduced (was 64) - Major speed gain, potential quality loss
    blockSize=3,        # Smallest possible odd number > 1 (was 5) - Speed gain
    uniquenessRatio=5,  # Reduced further (was 10)
    speckleWindowSize=25,# Reduced further (was 50)
    speckleRange=2,     # Keep
    disp12MaxDiff=15,   # Relaxed further (was 10)
    P1=2 * 3 * 3**2,    # Reduced P1/P2 further (was 4*..., 16*...)
    P2=8 * 3 * 3**2,
    mode=cv2.STEREO_SGBM_MODE_SGBM_3WAY # Keep fast mode
)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # --- 其他按键处理 ('g', 'f', 'x', 'r') ---
             ...
             if len(ground_points) >= 10: # 至少需要10个点
                 try:
                     # 调用优化后的函数，可以指定线程数或让其自动检测
                     new_normal = fit_ground_plane(ground_points, num_total_iterations=100) # 默认使用CPU核心数
                     # 或者 new_normal = fit_ground_plane(ground_points, num_threads=4) # 手动指定4个线程
                     ...
                     if new_normal is not None:
                         ...
                         print("拟合地面失败，未能计算出有效的法向量。")

                 except Exception as e: # Catch potential errors from fit_ground_plane itself
